...
void Table::ReadMeta(const leveldb::Footer &footer) {
...
Iterator* Table::BlockReader(void* arg, const ReadOptions& options, const Slice& index_value) {
    ...
    if (s.ok()) {
        ...
        if (block_cache != nullptr) {
            char cached_key_buffer[16];
            EncodeFixed64(cached_key_buffer, table->rep_->cache_id);
            EncodeFixed64(cached_key_buffer+8, handle.offset());
            Slice key(cached_key_buffer, sizeof(cached_key_buffer));
...
Iterator* Table::NewIterator(const leveldb::ReadOptions &options) const {
...
uint64_t Table::ApproximateOffsetOf(const leveldb::Slice &key) const {
