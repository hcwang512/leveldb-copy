...
Reader::Reader(leveldb::SequentialFile *file, Reporter *reporter, bool checksum,
               uint64_t initial_offset)
               : file_(file), reporter_(reporter), checksum_(checksum),
               backing_store_(new char[kBlockSize]), buffer_(),
               eof_(false), last_record_offset_(0), end_of_buffer_offset_(0),
    ...
    while (true) {
        ...
        switch (record_type) {
            ...
            default: {
                ...
                ReportCorruption(
                        (fragment.size() + (in_fragment_record ? scratch->size() : 0)),
                        buf);
                in_fragment_record = false;
                scratch->clear();
                break;
